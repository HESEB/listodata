(function(){
  ...
  const ADMIN_GROUPS=[
    {title:'DSS 2.0',desc:'데이터 중심 전환',items:[
      ...
      ['대표뉴스 날짜검수','./representative-news-date-review.html','게시일 누락·오류·미래 날짜 관리자 검수'],
    ...
    {title:'검수',desc:'분류 결과와 제외 자료 검토',items:[
      ...
      ['대표뉴스 날짜검수','./representative-news-date-review.html','게시일 누락·오류·미래 날짜 처리'],
  ...
  function addOperationsBadges(){if(document.getElementById('operations-badges-script'))return;const s=document.createElement('script');s.id='operations-badges-script';s.src='./operations-badges.js?v=phase-8-7';s.defer=true;document.body.appendChild(s);}
