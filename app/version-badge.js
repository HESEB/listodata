(() => {
  ...
  function ensureStyle(doc) {
    ...
    style.textContent = `
      ...
      .heseb-version-detail{position:fixed;right:16px;top:58px;z-index:9999;width:min(360px,calc(100vw - 32px));background:#fff;border:1px solid #dbe4ee;border-radius:16px;box-shadow:0 18px 40px rgba(15,23,42,.22);padding:14px;color:#172033;font-size:13px;display:none}
      ...
      .heseb-version-detail code{font-size:11px;background:#f1f5f9;border-radius:6px;padding:2px 5px}
  ...
  function render(data) {
    ...
    root.innerHTML = `
      ...
      <div class="heseb-version-detail" id="heseb-version-detail">
        ...
        <div class="row"><b>Cache</b><span><code>${fmt(data.cache_bust)}</code></span></div>
      </div>
    `;
    ...
    // Replace old static version labels when possible without touching layout.
    Array.from(doc.querySelectorAll('small, .version, [class*=version], [id*=version]')).forEach((el) => {
      const text = (el.textContent || '').trim();
      if (/^v\d+\.\d+|v3\.3\.2|v4\.10/i.test(text)) {
        el.textContent = display.short_label || data.version || text;
        el.title = `${fmt(build)} / ${fmt(data.data_updated_at)}`;
      }
    });
  }
  ...
  async function init() {
    try {
      ...
      render({ version: 'v6.2.0', phase: 'Phase 6-2', display: { label: 'v6.2.0 · version pending', status_label: '대기' }, workflow: { status: 'pending' } });
    }
  }
