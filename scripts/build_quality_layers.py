...
# Keep this JSON-serializable. Do not use set() here.
SCORE_AXES = {
...
def build_clean(raw_items: list[dict]) -> tuple[list[dict], list[dict]]:
    ...
    for item in raw_items:
        ...
        row.update({
            ...
            "impact_score": impact,
            "quality_score": q,
            "quality_policy": "phase2_quality_v2_robust",
        })
        if q < 25:
...
def build_analysis(clean_items: list[dict]) -> tuple[dict, dict, dict]:
    ...
    for sp, st in stats.items():
        ...
        score_items.append({
            ...
            "reason": reason,
            "score_policy": "phase2_evidence_score_v2_robust",
        })
        chain_items.append({
            "id": sp,
            "name": SPECIES_LABEL.get(sp, sp),
    ...
    return {"updated_at": now_iso(), "policy": "phase2_evidence_score_v2_robust", "score_axes": serializable_axes(), "species": score_items}, {"updated_at": now_iso(), "policy": "phase2_evidence_chain_v2_robust", "items": chain_items}, matrix
...
def build_admin_log(raw_items: list[dict], clean_items: list[dict], rejected: list[dict]) -> dict:
    ...
    return {
        "updated_at": now_iso(),
        "policy": "phase2_admin_log_v2_robust",
        "summary": {
